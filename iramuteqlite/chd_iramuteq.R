...
construire_stats_classes_iramuteq <- function(dfm_obj, classes, max_p = 1, stats_mode = c("vectorise", "classique"), n_cores = NULL) {
  ...
  docs_par_terme <- Matrix::colSums(mat > 0)
  occ_par_terme <- Matrix::colSums(mat)
  occ_par_classe <- rowsum(as.numeric(Matrix::rowSums(mat)), group = classes, reorder = FALSE)
  ...
  construire_stats_classe <- function(cl) {
    ...
    docs_terme_cl <- Matrix::colSums(mat[in_cl, , drop = FALSE] > 0)
    docs_terme_hors <- pmax(0, docs_par_terme - docs_terme_cl)

    occ_terme_cl <- Matrix::colSums(mat[in_cl, , drop = FALSE])
    ...
    freq_cl <- Matrix::colSums(mat[in_cl, , drop = FALSE])
